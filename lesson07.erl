...
handle_event(#wx{event = #wxKey{keyCode = KeyCode}}, State) ->
  NewState = case KeyCode of
               ...
               $I ->
                 State#state{zindex = State#state.zindex + 0.02};
               $O ->
                 State#state{zindex = State#state.zindex - 0.02};
               ?WXK_UP ->
                 State#state{xspeed = State#state.xspeed + 0.01};
               ?WXK_DOWN ->
                 State#state{xspeed = State#state.xspeed - 0.01};
               ?WXK_LEFT ->
                 State#state{yspeed = State#state.yspeed + 0.01};
               ?WXK_RIGHT ->
                 State#state{yspeed = State#state.yspeed - 0.01};
               _ ->
                 State
...
setup_gl(#state{parent = Window} = State) ->
  ...
  gl:lightfv(?GL_LIGHT1, ?GL_AMBIENT, State#state.light_ambient),
  gl:lightfv(?GL_LIGHT1, ?GL_DIFFUSE, State#state.light_diffuse),
  gl:lightfv(?GL_LIGHT1, ?GL_POSITION, State#state.light_position),
  gl:enable(?GL_LIGHT1),

  Image = wxImage:new("crate.jpg"),
  TextureNear = load_texture_by_image(Image, ?GL_NEAREST, ?GL_NEAREST),
  TextureLinear = load_texture_by_image(Image, ?GL_LINEAR, ?GL_LINEAR),
  TextureMipMap = load_texture_by_image(Image, ?GL_LINEAR, ?GL_LINEAR_MIPMAP_NEAREST),
  build_mipmap_by_image(Image),

  State#state{texture_near = TextureNear, texture_linear = TextureLinear, texture_mipmap = TextureMipMap}.
...
draw(#state{xrot = XRot, yrot = YRot} = State) ->
  gl:clear(?GL_COLOR_BUFFER_BIT bor ?GL_DEPTH_BUFFER_BIT),
  gl:loadIdentity(),
  gl:translatef(0.0, 0.0, State#state.zindex),

  gl:rotatef(XRot, 1.0, 0.0, 0.0),
  gl:rotatef(YRot, 0.0, 1.0, 0.0),

  Texture = State#state.texture_linear,
  ...
  State#state{xrot = XRot + State#state.xspeed, yrot = YRot + State#state.yspeed}.

load_texture_by_image(Image, FilterMag, FilterMin) ->
...
build_mipmap_by_image(Image) ->
  ...
  glu:build2DMipmaps(?GL_TEXTURE_2D, Format, Width, Height, Format, ?GL_UNSIGNED_BYTE, Data).
